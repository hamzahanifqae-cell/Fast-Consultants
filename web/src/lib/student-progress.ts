...
export const STUDENT_PROGRESS_BREAKDOWNS: Record<StudentSectionKey, readonly string[]> = {
  personal: ['Personal', 'Education', 'Job', 'Other'],
  documents: ['Upload', 'Review', 'Approved'],
  universities: ['Waiting', 'Options', 'Required docs'],
  fees: ['Waiting', 'Pay', 'Approved'],
  interview: ['Locked', 'Prep', 'Meeting'],
  visa: ['Waiting', 'Scheduled', 'Completed'],
  status: ['Docs', 'Fees', 'Prep', 'Interview', 'File Making'],
};
...
export function journeyProgressPalette(percent: number): JourneyProgressPalette {
  ...
  return {
    ...
    fillEnd,
    track: `${fillEnd}22`,
    fillGradient: `linear-gradient(90deg, ${fillStart}, ${fillEnd})`,
  };
}
